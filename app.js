...
calculateTotal() {
    let cartTotal = cart.reduce((total, item) => total + item.price * item.amount, 0);
    if (cartTotal > 1000) {
        cartTotal *= 0.9; // Apply 10% discount
    }
    cartTotal = cartTotal.toFixed(2);
    cartTotalDOM.textContent = `Total: ${cartTotal} บาท`;
}
...
addToCart(id) {
    ...
    if (inCart) {
        ...
        let cartItem = {...selectedProduct, amount: 1};
        cart.push(cartItem);
    }
    
    // Update cart total and UI
    this.calculateTotal();
